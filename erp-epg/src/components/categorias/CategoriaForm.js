...
export function CategoriaForm({ categoria = null, rubros }) {
  ...
  if (!rubrosOpciones.length) {
    return (
      <div className="card max-w-xl border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-950">
        ...
        <Link
          href="/rubros/nuevo"
          className="mt-3 inline-flex font-medium text-primary underline underline-offset-2"
        >
          Ir a crear un rubro
        </Link>
      </div>
    ...
    <form action={formAction} className="card max-w-xl p-5 md:p-6">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-1.5">
          ...
          >
            Rubro <span className="text-primary">*</span>
          </label>
          <select
            id="id_rubro"
            name="id_rubro"
