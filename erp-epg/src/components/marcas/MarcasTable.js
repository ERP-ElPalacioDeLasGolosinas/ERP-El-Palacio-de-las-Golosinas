...
import { habilitarMarca, inhabilitarMarca } from "@/lib/marcas/actions";
 ...
 * }} props
 */
export function MarcasTable({ marcas, incluirInactivas }) {
  ...
  const filtradas = useMemo(() => {
    ...
    if (!q) return marcas;
    return marcas.filter((m) => m.nombre_marca.toLowerCase().includes(q));
  }, [marcas, busqueda]);
  ...
  function manejarErrorAccion(result) {
    ...
    if (result.code === "MRC03") router.refresh();
  }

  function toggleActivo(marca) {
    startTransition(async () => {
      const result = marca.activo
        ? await inhabilitarMarca(marca.id_marca)
        : await habilitarMarca(marca.id_marca);
      if (!result.ok) {
        ...
        <div className="palacio-card overflow-hidden">
          <div className="flex items-center justify-between border-b border-palacio-border px-5 py-3">
            ...
            <span className="text-xs text-palacio-muted">
              {filtradas.length} marca{filtradas.length === 1 ? "" : "s"}
            </span>
          </div>
