...
/**
 * Registra un ingreso por compra (recepción de mercadería) como un lote
 * asociado a una factura existente, vía RPC `fn_lote_registrar_desde_comprobante`.
 * Crea `inventario` / `inventario_producto` y un movimiento "ingreso por compra"
 * por producto para reflejar `stock`.
 *
 * @param {{
 ...
 * @returns {Promise<{ ok: boolean, error: string | null, code?: string | null, data?: unknown }>}
 */
export async function registrarIngresoPorComprobante({
  id_comprobante,
  id_deposito,
  detalle_lote,
  productos,
  ...
  const { data, error } = await supabase.rpc(
    "fn_lote_registrar_desde_comprobante",
    {
      p_id_comprobante: id_comprobante || null,
